  ...
  switch (modifiedPath[3]) {
    case 'vertices':
      if (modifiedPath[5] !== 'selected') {
        var vertex = layer.vertices.get(modifiedPath[4]);
        ...
        if (modifiedPath[5] === 'areas') {
          var areaID = vertex.areas.get(~~modifiedPath[6]);
    ...
    case 'holes':
      ...
      break;
    case 'lines':
      var line = layer.lines.get(modifiedPath[4]);
      ...
      break;
    case 'areas':
      var area = layer.areas.get(modifiedPath[4]);
      ...
      break;
    case 'items':
      var item = layer.items.get(modifiedPath[4]);
...
function updateHole(sceneData, oldSceneData, planData, layer, holeID, differences, catalog, holesActions, selfDestroy, selfBuild) {
  ...
  var oldHole = oldSceneData.layers.get(layer.id).holes.get(holeID);
...
function updateLine(sceneData, oldSceneData, planData, layer, lineID, differences, catalog, linesActions, selfDestroy, selfBuild) {
  ...
  var line = layer.lines.get(lineID);
  var oldLine = oldSceneData.layers.get(layer.id).lines.get(lineID);
...
function updateArea(sceneData, oldSceneData, planData, layer, areaID, differences, catalog, areaActions, selfDestroy, selfBuild) {
  ...
  var area = layer.areas.get(areaID);
  var oldArea = oldSceneData.layers.get(layer.id).areas.get(areaID);
