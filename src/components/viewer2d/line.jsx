...
export default function Line({line, vertices, holes}) {
  ...
  let holesComp = line.holes.map(holeID => {
    ...
    let holeStyle = hole.selected ? STYLE_HOLE_BASE : STYLE_HOLE_BASE;
    return (
  ...
  let linePath = `M${0} ${ -epsilon}  L${lenght} ${-epsilon}  L${lenght} ${epsilon}  L${0} ${epsilon}  z`;
  return (
    <g transform={`translate(${x1}, ${y1}) rotate(${angle}, 0, 0)`}>
      <path d={linePath} style={line.selected ? STYLE_SELECTED : STYLE_BASE}/>
      {holesComp}
    </g>
  );
