...
export default {
  name: "finestra_veneziana",
  prototype: "holes",

  info: {
    tag: ['window veneziana'],
    group: "Vertical Closing",
    title: "finestra con veneziana",
    description: "finestra con veneziana",
    image: require('./venetian.png')
  },
  ...
  render3D: function (element, layer, scene) {
    ...
    function makeBlind() {
      ...
      let venetianBlind = new Three.Object3D();

      //colors
      let white = new Three.MeshLambertMaterial( {color: 0xffffff, opacity:0.5, transparent:true} );
      ...
      for (let i = 0; i < 25; i+=.7) {
        ...
        venetianBlind.add(mesh);

      }

      for (let j = -1.25; j > -19; j+=-4) {

        let geometry1 = new Three.CylinderGeometry( 0.105, 0.105, 26, 32 );
        let tube = new Three.Mesh(geometry1,white);
        tube.position.set(j,12.5,.35);
        venetianBlind.add(tube);
      ...
      for (let k = -.5; k < 27; k+=26) {
        ...
        let mesh2 = new Three.Mesh( geometry2, grey ) ;
        mesh2.position.set(0,k,1);
        mesh2.rotation.z+=Math.PI/2;
        mesh2.rotation.x+=-Math.PI/2;
        venetianBlind.add(mesh2);
      }

      let valueObject = new Three.Box3().setFromObject(venetianBlind);
      ...
      let deltaZ = Math.abs(valueObject.max.z - valueObject.min.z);
       ...
       venetianBlind.position.x+=width1/.025;
       venetianBlind.position.y+=-height1/.4;
       venetianBlind.scale.set(5.2*width1 / deltaZ, 5.45*height1 / deltaY, 2.5*thickness / deltaX);
