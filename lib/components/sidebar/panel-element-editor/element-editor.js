  ...
  _createClass(ElementEditor, [{
    ...
    value: function initPropData(element, layer, state) {
      ...
      for (var name in catalogElement.properties) {
        mapped[name] = new _immutable.Map({
          currentValue: element.properties.has(name) ? element.properties.get(name) : (0, _immutable.fromJS)(curr.defaultValue),
        ...
        case 'holes':
          {
            ...
            if (attributeName === 'offsetA') {
              ...
              var _xp = x1 - (lengthValue + this.props.element.properties.get('width').get('length') / 2) * Math.cos(alpha);
              var _yp = y1 - (lengthValue + this.props.element.properties.get('width').get('length') / 2) * Math.sin(alpha);

              offset = geometry.pointPositionOnLineSegment(x0, y0, x1, y1, _xp, _yp);

              var startAt = math.toFixedFloat(lineLength * offset - this.props.element.properties.get('width').get('length') / 2, 2);
    ...
    value: function render() {
      ...
      return _react2.default.createElement(
        ...
        _react2.default.createElement(_attributesEditor2.default, { element: this.props.element,
