...
var Viewer3DFirstPerson = function (_React$Component) {
  ...
  _createClass(Viewer3DFirstPerson, [{
    ...
    value: function componentDidMount() {
      ...
      var humanHeightPixels = (0, _convertUnits2.default)(humanHeight.length).from(humanHeight.unit).to(state.scene.unit);
      ...
      this.removeObjectKeyListener = function (event) {
        var KEY_DELETE = 46;
        var KEY_BACKSPACE = 8;

        var selectedLayer = _this2.props.state.scene.selectedLayer;
        var selectedObjects = _this2.props.state.scene.layers.get(selectedLayer).selected;
        switch (event.keyCode) {
          case KEY_BACKSPACE:
          case KEY_DELETE:

            selectedObjects.lines.forEach(function (line) {
              editingActions.remove(line);
            });

            selectedObjects.holes.forEach(function (hole) {
              editingActions.remove(hole);
            });

            selectedObjects.items.forEach(function (item) {
