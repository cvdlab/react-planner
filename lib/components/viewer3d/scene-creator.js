...
function addHole(sceneData, planData, layer, holeID, catalog, editingActions) {
  ...
  catalog.getElement(holeData.type).render3D(holeData, layer, sceneData).then(function (object) {
    ...
    var holeAltitude = (0, _convertUnits2.default)(holeData.properties.get('altitude').get('length')).from(holeData.properties.get('altitude').get('unit')).to(sceneData.unit) * sceneData.pixelPerUnit;

    var holeHeight = (0, _convertUnits2.default)(holeData.properties.get('height').get('length')).from(holeData.properties.get('height').get('unit')).to(sceneData.unit) * sceneData.pixelPerUnit;
