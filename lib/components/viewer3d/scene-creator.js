...
function parseData(sceneData, editingActions, catalog) {
  ...
  sceneData.layers.forEach(function (layer) {
    ...
    layer.lines.forEach(function (line) {
      ...
      var wall = createWall(layer, line, editingActions, catalog);
      plan.add(wall);
  ...
  var grid = (0, _gridCreator2.default)(sceneData.width, sceneData.height, sceneData.pixelPerUnit);
...
function createWall(layer, line, editingActions, catalog) {
  ...
  var wall = catalog.getElement(line.type).render3D(line, layer);

  var distance = Math.sqrt(Math.pow(vertex0.x - vertex1.x, 2) + Math.pow(vertex0.y - vertex1.y, 2));
  var bevelRadius = line.properties.get('thickness');
  ...
  line.holes.forEach(function (holeID) {
    ...
    var holePromise = catalog.getElement(holeData.type).render3D(holeData, undefined);

    holePromise.then(function (object) {
      var boundingBox = new _three2.default.Box3().setFromObject(object);
      var center = [(boundingBox.max.x - boundingBox.min.x) / 2 + boundingBox.min.x, (boundingBox.max.y - boundingBox.min.y) / 2 + boundingBox.min.y, (boundingBox.max.z - boundingBox.min.z) / 2 + boundingBox.min.z];

      var coordinates = [(distance - bevelRadius) * holeData.offset, holeData.properties.get('altitude') + holeData.properties.get('height') / 2, 0];
...
function replaceLine(layer, oldLineObject, newLineData, editingActions, planData, isVisible, catalog) {
  ...
  var newLineObject = createWall(layer, newLineData, editingActions, catalog);
...
function createItem(layer, item, editingActions, sceneGraph, catalog, plan) {
  var item3DPromise = catalog.getElement(item.type).render3D(item, layer);

  item3DPromise.then(function (item3D) {

    var boundingBox = new _three2.default.Box3().setFromObject(item3D);
...
function replaceItem(layer, oldItemObject, newItemData, editingActions, planData, catalog) {
  ...
  var item3DPromise = catalog.getElement(newItemData.type).render3D(newItemData, layer);
