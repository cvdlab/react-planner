...
function mergePlugins(plugins) {
  ...
  customReducer.length > 0 ? customReducer = funcChain(customReducer) : function (store) {
    return store;
  };
  onReady = onReady.length > 0 ? funcChain(onReady) : function () {
    return null;
  };

  return { customActions: customActions, toolbarButtons: toolbarButtons, customContents: customContents, customReducer: customReducer, onReady: onReady };
}