...
function addLineAvoidingIntersections(layer, type, x0, y0, x1, y1, catalog) {
  ...
  layer = layer.withMutations(function (layer) {
    ...
    lines.forEach(function (line) {
      ...
      if (!(v0.x === x0 && v0.y === y0 || v0.x === x1 && v0.y === y1 || v1.x === x0 && v1.y === y0 || v1.x === x1 && v1.y === y1)) {
...
function setProperties(layer, prototype, ID, properties) {
  properties = (0, _immutable.Map)(properties);
...
/** images features **/
function addImage(layer, uri, x0, y0, x1, y1) {
  var image = void 0;

  layer = layer.withMutations(function (layer) {
    var imageID = _idBroker2.default.acquireID();

    var v0 = void 0,
        v1 = void 0;

    var _addVertex5 = addVertex(layer, x0, y0, 'images', imageID);

    layer = _addVertex5.layer;
    v0 = _addVertex5.vertex;

    var _addVertex6 = addVertex(layer, x1, y1, 'images', imageID);

    layer = _addVertex6.layer;
    v1 = _addVertex6.vertex;


    image = new _models.Image({
      id: imageID,
      vertices: new _immutable.List([v0.id, v1.id]),
      uri: uri
    });
...
function loadLayerFromJSON(json, catalog) {
  ...
  var loadLine = function loadLine(line) {
    return new _models.Line(line).set('type', catalog.getElement(line.type).name).set('vertices', new _immutable.List(line.vertices)).set('holes', new _immutable.List(line.holes)).set('properties', new _immutable.Map(line.properties || {}));
  };

  var loadHole = function loadHole(hole) {
    return new _models.Hole(hole).set('type', catalog.getElement(hole.type).name).set('properties', new _immutable.Map(hole.properties || {}));
  };

  var loadArea = function loadArea(area) {
    return new _models.Area(area).set('type', catalog.getElement(area.type).name).set('vertices', new _immutable.List(area.vertices)).set('properties', new _immutable.Map(area.properties || {}));
  };

  var loadImage = function loadImage(image) {
    return new _models.Image(image).set('vertices', new _immutable.List(image.vertices));
  };

  var loadItem = function loadItem(item) {
    return new _models.Item(item);
  };
  ...
  var loadLayer = function loadLayer(layer) {
    ...
    }).toMap()).set('images', new _immutable.Seq(layer.images).map(function (image) {
