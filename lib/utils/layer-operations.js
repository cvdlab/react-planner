  ...
  layer = layer.withMutations(function (layer) {
    ...
    innerCyclesByVerticesID.forEach(function (cycle, ind) {
      ...
      if (areaInUse) {
        ...
        layer = layer.setIn(['areas', areasID[ind], 'holes'], new _immutable.List());
      } else {
    ...
    for (var _x2 = 0; _x2 < uniqueCycleContaining.length; _x2++) {
      layer = layer.setIn(['areas', areasID[_x2], 'holes'], new _immutable.List(uniqueCycleContaining[_x2] ? uniqueCycleContaining[_x2].map(function (el) {
        return areasID[el];
      }) : []));
    }
  });
