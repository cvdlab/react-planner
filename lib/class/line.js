  ...
  _createClass(Line, null, [{
    ...
    value: function createAvoidingIntersections(state, layerID, type, x0, y0, x1, y1, oldProperties, oldHoles) {
      ...
      state = state.getIn(['scene', 'layers', layerID, 'lines']).reduce(function (reducedState, line) {
        ...
        var hasCommonEndpoint = _export2.GeometryUtils.samePoints(v0, initialPoints[0]) || _export2.GeometryUtils.samePoints(v0, initialPoints[1]) || _export2.GeometryUtils.samePoints(v1, initialPoints[0]) || _export2.GeometryUtils.samePoints(v1, initialPoints[1]);
        var intersection = _export2.GeometryUtils.twoLineSegmentsIntersection(initialPoints[0], initialPoints[1], v0, v1);
        if (intersection.type === 'colinear') {
          if (!oldHoles) {
            oldHoles = [];
          }
          var orderedVertices = _export2.GeometryUtils.orderVertices(initialPoints);
    ...
    value: function endDrawingLine(state, x, y) {
      ...
      state = _export.Layer.mergeEqualsVertices(state, layerID, vertexID).updatedState;
      state = Line.remove(state, layerID, lineID).updatedState;
